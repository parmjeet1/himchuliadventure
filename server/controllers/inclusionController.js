...
InclusionModel=require("../models/InclusionModel");

const addInclusion = async (req, res) => {
  try {
    const { description, destinationId} = req.body;


    const inclusion = new InclusionModel({description,destinationId});
...
if(!id){ return res.status(400).json({error:"id could not foud"}) }
    ...
    const inclusion=await InclusionModel.find({destinationId:id}).select("-updatedBy -createdAt -updatedAt");
