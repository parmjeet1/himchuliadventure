...
export default function ImageUploader({ packageId }) {
  ...
  const [showViewPackage, setShowViewPackage] = useState(false); // NEW STATE
  ...
  const handleUpload = async () => {
    if (!packageId) {
      setMessage("Error: Package ID is missing.");
      return;
    }
    ...
    try {
      ...
      setSelectedImagePath(null);
      setPreview(null);
      setShowViewPackage(true); // SHOW VIEW BUTTON AFTER SUCCESS
    } catch (error) {
  ...
  return (
    <div className="flex flex-col items-center p-6 bg-gray-100 rounded-lg shadow-md w-full max-w-md mx-auto mt-20">
      ...
      {/* View Package Button (Visible after Upload) */}
      {showViewPackage && (
        <button
          onClick={() => router.push("/admin/all-treks")}
